...
struct CallLog {
    ...
    CallLog(R result_, Args... args)
        : result(std::move(result_)), arguments(std::move(args)...) {
    }

    CallLog(R result_, std::tuple<Args...> arguments_)
        : result(std::move(result_)), arguments(std::move(arguments_)) {
    }
};
...
struct function_traits<R (*)(Args...)> {
    using call_result_type =
        CallLog<std::remove_reference_t<R>, std::remove_reference_t<Args>...>;
};

}  // namespace impl

template <typename Fn,
          typename C = typename impl::function_traits<Fn>::call_result_type>
struct Memorizer {
    Fn fn;
